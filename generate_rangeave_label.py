...
def generate_rangemin_and_create_dictionary(dataset): 
    """
    ...
    calculated_range_min_val = np.mean(range_values_slice)
    ...
    result_dictionary = {
        **dataset, # 元のデータセットのキーをすべて保持
        ...
        'calculated_range_min': calculated_range_min_val, 
        'range_start_index': int(start_index),      
        'range_end_index': int(end_index),       
    }
...
if __name__ == "__main__":
    ...
    if not datasets_from_file:
        ...
        output_jsonl_file = "rangemin_output.jsonl"
        try:
